...
#     print('Squeezed query_points', unstacked_query_points[0].shape, unstacked_query_points[1].shape)
    ...
    for dim in index_order:
        ...
        max_floor = torch.tensor(size_in_indexing_dimension - 2, dtype=query_type).cuda()
        min_floor = torch.tensor(0.0, dtype=query_type).cuda()
        maxx = torch.max(min_floor, torch.floor(queries).cuda())
        floor = torch.min(maxx, max_floor).cuda()
        ...
        alpha = (queries.cuda() - floor).clone().detach().cuda()
        min_alpha = torch.tensor(0.0, dtype=grid_type).cuda()
        max_alpha = torch.tensor(1.0, dtype=grid_type).cuda()
    ...
    def gather(y_coords, x_coords, name):
        linear_coordinates = batch_offsets.cuda() + y_coords * width + x_coords
        gathered_values = torch.gather(flattened_grid.t().cuda(), 1, linear_coordinates).cuda()
